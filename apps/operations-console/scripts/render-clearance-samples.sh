#!/usr/bin/env bash
# Generate sample port cyber clearance HTML for operations-console dist/.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CATENA_ROOT="$(cd "${ROOT}/../.." && pwd)"
EDS="${EDS_BIN:-${CATENA_ROOT}/../edgesentry-rs/target/debug/eds}"
...
if [[ ! -x "$EDS" ]]; then
  ...
  (cd "${CATENA_ROOT}/../edgesentry-rs" && cargo build -p eds)
  EDS="${CATENA_ROOT}/../edgesentry-rs/target/debug/eds"
fi
