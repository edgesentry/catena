"""W5 — operations-console clearance UI contract (templates, field map, dist HTML)."""
...
OC_ROOT = Path(__file__).resolve().parents[2] / "apps" / "operations-console"
FIELD_MAP = OC_ROOT / "field_maps" / "port_cyber_clearance_field_map.json"
FIXTURES = OC_ROOT / "fixtures" / "clearance"
DIST = OC_ROOT / "dist"
...
def test_template_spec_exists() -> None:
    spec = OC_ROOT / "templates" / "port-cyber-clearance.md"
    assert spec.is_file()
    text = spec.read_text(encoding="utf-8")
    assert "port-cyber-clearance" in text
    assert "field_map" in text.lower()


def test_dist_sample_html_bundled() -> None:
    hold_html = DIST / "vessel-hold_port-cyber-clearance.html"
    clean_html = DIST / "vessel-clean_port-cyber-clearance.html"
    assert hold_html.is_file(), "missing bundled dist HTML"
...
def test_eds_render_clearance_smoke(tmp_path: Path) -> None:
    ...
    if probe.returncode != 0:
        pytest.skip("eds lacks document render-clearance")
