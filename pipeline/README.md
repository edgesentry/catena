...
| Path | Role |
|------|------|
| `pipeline/maritime_cyber/` | Graph build, rule evaluation, fleet demo, audit refs |
...
| `agents/port_clearance/` | W6 E2E runner (`run_clearance.py`), WORM store, narratives |
...
Repo root for `_REPO_ROOT` in `pipeline/maritime_cyber/*` is **`catena/`** (not a nested `pipeline/pipelines/` tree).
...
- `../edgesentry-rs` — `eds document render-clearance`, `eds audit sign-clearance` (called from `agents/port_clearance/`)
- `../documaris/dist` — HTML bundles from `export_vessel_graph.py`
- `../edgesentry-commercial/docs/programs/.../submission/artefacts` — CapVista artefacts
